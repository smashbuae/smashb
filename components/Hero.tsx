    ...
    <section className="relative h-screen w-full overflow-hidden flex flex-col items-center justify-between bg-smash-brown pt-20 pb-12 px-6">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden bg-gradient-to-b from-[#1a1110] via-[#2a1a18] to-[#12002b]">
        ...
        {[...Array(6)].map((_, i) => (
          <motion.div
            key={`cloud-far-${i}`}
            initial={{ x: -300, y: (i * 20) + "%", opacity: 0 }}
            animate={{ 
              x: '120vw', 
              opacity: [0, 0.08, 0],
            }}
            ...
            className="absolute w-[500px] h-[300px] bg-smash-cream rounded-full blur-[120px] pointer-events-none"
          />
        ))}
        ...
        {Array.from({ length: 40 }).map((_, i) => (
          <motion.div
            ...
            className="absolute h-[1.5px] w-64 bg-gradient-to-r from-transparent via-smash-red/30 to-transparent rotate-[3deg]"
          />
        ))}
        ...
        {activePopup && (
          ...
          >
            ...
            >
              <button 
                onClick={() => setActivePopup(null)}
                className="absolute top-4 right-4 text-smash-cream hover:text-smash-red font-primary text-2xl transition-colors"
                aria-label="Close"
              >
                ✕
              </button>
